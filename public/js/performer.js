...
pcAudio.onicecandidate = async (e) => {
  console.log(
    `ICE candidate (pcAudio) ${e.candidate && e.candidate.candidate}`
  );
  if (e.candidate == null) {
    pcAudioSendAnswer();
  }
...
pcAudio.ondatachannel = function (e) {
  ...
  dc.onmessage = (e) => {
    ...
    switch (msg.type) {
      case "AnimationEvent":
        document.getElementById(`${msg.objectName}-current-event`).innerText =
          msg.stateName;
        $(`#${msg.objectName}AnimationArea button`).removeClass(
          "active-animation"
        );
        let currentState =
          MAYFLY_STATE_TO_BUTTON_DICT[msg.stateName] ||
          BEAVERDAM_STATE_TO_BUTTON_DICT[msg.stateName] ||
          BUILDINGS_STATE_TO_BUTTON_DICT[msg.stateName];
        if (currentState) {
          $(`#${msg.objectName}AnimationArea button#${currentState}`).addClass(
            "active-animation"
          );
        }
        break;
...
pcVideo.onicecandidate = async (e) => {
  console.log(
    `ICE candidate (pcVideo) ${e.candidate && e.candidate.candidate}`
  );
  if (e.candidate == null) {
    pcVideoSendAnswer();
  }
...
document.getElementById("Idle").onclick = () => {
  dc && dc.send(JSON.stringify({ type: "MayflyAnimation", content: "Idle" }));
};
document.getElementById("Sit").onclick = () => {
  dc && dc.send(JSON.stringify({ type: "MayflyAnimation", content: "Sit" }));
};
document.getElementById("Surprised").onclick = () => {
  dc &&
    dc.send(JSON.stringify({ type: "MayflyAnimation", content: "Surprised" }));
};
document.getElementById("Concerned").onclick = () => {
  dc &&
    dc.send(JSON.stringify({ type: "MayflyAnimation", content: "Concerned" }));
};
document.getElementById("GestureArmsUp").onclick = () => {
  dc &&
    dc.send(
      JSON.stringify({ type: "MayflyAnimation", content: "GestureArmsUp" })
    );
};
document.getElementById("GestureArmsForward").onclick = () => {
  dc &&
    dc.send(
      JSON.stringify({ type: "MayflyAnimation", content: "GestureArmsForward" })
    );
};
document.getElementById("Book").onclick = () => {
  dc && dc.send(JSON.stringify({ type: "MayflyAnimation", content: "Book" }));
};

document.getElementById("BeaverDamOff").onclick = () => {
  dc &&
    dc.send(
      JSON.stringify({ type: "BeaverDamAnimation", content: "BeaverDamOff" })
    );
};
document.getElementById("BeaverDamOn").onclick = () => {
  dc &&
    dc.send(
      JSON.stringify({ type: "BeaverDamAnimation", content: "BeaverDamOn" })
    );
};

document.getElementById("BuildingsOff").onclick = () => {
  dc &&
    dc.send(
      JSON.stringify({ type: "BuildingsAnimation", content: "BuildingsOff" })
    );
};
document.getElementById("BuildingsOn").onclick = () => {
  dc &&
    dc.send(
      JSON.stringify({ type: "BuildingsAnimation", content: "BuildingsOn" })
    );
};
